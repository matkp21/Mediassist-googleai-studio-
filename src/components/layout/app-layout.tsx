...
export function AppLayout({ children }: { children: ReactNode }) {
  ...
  const [displayState, setDisplayState] = useState<'loading' | 'onboarding' | 'welcome' | 'app'>('app');
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (!onboardingComplete && user) {
        setTimeout(() => setDisplayState('onboarding'), 0);
      } else if (user && !welcomeShownThisSession && !['/login', '/signup'].includes(pathname)) {
        setTimeout(() => setDisplayState('welcome'), 0);
      } else {
        setTimeout(() => setDisplayState('app'), 0);
      }
    }
