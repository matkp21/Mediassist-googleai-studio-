...
import { Stethoscope, ScanSearch, BookOpenText, LayoutDashboard, Activity, Clock } from "lucide-react";
import { useProMode } from "@/contexts/pro-mode-context";

export type ActiveMode = 'symptom' | 'image' | 'education' | 'dashboard' | 'diagnostic' | 'recent';
...
export function ModeSwitcher({ activeMode, setActiveMode }: ModeSwitcherProps) {
  ...
  const modes = useMemo(() => {
    if (userRole === 'pro') {
      return [
        { id: 'diagnostic' as const, label: 'Diagnostic Tools', icon: Activity, ariaLabel: 'Switch to Diagnostic Tools' },
        { id: 'image' as const, label: 'Image Processing', icon: ScanSearch, ariaLabel: 'Switch to Image Processing mode' },
        { id: 'recent' as const, label: 'Recent Tools', icon: Clock, ariaLabel: 'Switch to Recent Tools' },
      ];
    }
